...
class Bot:
    def __init__(self, config: dict):
        self.spot = False  # This bot is not for spot trading
        self.config = config  # Load the bot's configuration
        self.config["max_leverage"] = 25  # Set maximum leverage to 25
        self.xk = {}  # Initialize an empty dictionary for exchange-specific data

        self.ws_user = None  # Placeholder for the user websocket client
        self.ws_market = None  # Placeholder for the market websocket client

        self.hedge_mode = self.config["hedge_mode"] = True  # Enable hedge mode by default
        self.set_config(self.config)  # Apply any necessary config adjustments
        self.server_time = utc_ms()  # Get the current server time in milliseconds

        # Dictionary to store timestamps when actions are locked
        self.ts_locked = {
            k: 0.0
            for k in [
                "cancel_orders",
        ...
        # Dictionary to store timestamps when locks are released
        self.ts_released = {k: 1.0 for k in self.ts_locked}
        # Dictionary to track error halt states
        self.error_halt = {
            "update_open_orders": False,
            "update_fills": False,
            "update_position": False,
        }
        self.heartbeat_ts = 0  # Timestamp of the last heartbeat
        self.heartbeat_interval_seconds = 60 * 60  # Heartbeat interval (1 hour)
        self.listen_key = None  # Variable for storing Binance listen key (if applicable)

        self.position = {}  # Dictionary to store current position data
        self.open_orders = []  # List to store open orders
        self.fills = []  # List to store fill data
        # Dictionary to store timestamps of last fills for different order types
        ...
        self.price = 0.0  # Current market price
        self.ob = [0.0, 0.0]  # Order book, [bid price, ask price]
        self.emas_long = np.zeros(3)  # Array to store long EMA values
        self.emas_short = np.zeros(3)  # Array to store short EMA values
        self.ema_sec = 0  # Second when the last EMA was calculated
        self.auto_unstuck_on_timer = True  # Enable automatic unstuck on timer

        self.n_orders_per_execution = 2  # Maximum number of orders to create per execution
        self.delay_between_executions = 3  # Delay in seconds between order executions
        self.force_update_interval = 30  # Interval in seconds for forced updates

        self.c_mult = self.config["c_mult"] = 1.0  # Multiplier for contract size

        # Define the log file path
        self.log_filepath = make_get_filepath(f"logs/{self.exchange}/{config['config_name']}.log")

        # Load API keys from config or set to None if not provided
        self.api_keys = config["api_keys"] if "api_keys" in config else None
        # Load exchange API key, secret, and passphrase
        _, self.key, self.secret, self.passphrase = load_exchange_key_secret_passphrase(
            self.user, self.api_keys
        )
        self.broker_code = load_broker_code(self.exchange)  # Load broker code

        self.log_level = 0  # Set initial log level

        self.user_stream_task = None  # Task for handling user stream
        self.market_stream_task = None  # Task for handling market stream

        self.stop_websocket = False
        self.process_websocket_ticks = True

        self.custom_id_max_length = 36
        self.trailing_stop_price_long = 0.0  
        self.trailing_stop_percentage_long = 0.0
        self.trailing_stop_price_short = float('inf') 
        self.trailing_stop_percentage_short = 0.0
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        required=False,
        dest="trailing_stop_percentage_long", 
        default=0.0, 
        help="Trailing stop percentage for long positions (e.g., 2.0 for 2%). Set to 0.0 to disable."
    )
    parser.add_argument(
        "-tss",
        "--trailing-stop-short",
        type=float,
        required=False,
        dest="trailing_stop_percentage_short",
        default=0.0,
        help="Trailing stop percentage for short positions (e.g., 2.0 for 2%). Set to 0.0 to disable."
    )
    ...
    # Initialize trailing stop prices in your bot instance
    bot.trailing_stop_price_long = 0.0  
    bot.trailing_stop_percentage_long = args.trailing_stop_percentage_long
    bot.trailing_stop_price_short = float('inf') 
    bot.trailing_stop_percentage_short = args.trailing_stop_percentage_short
